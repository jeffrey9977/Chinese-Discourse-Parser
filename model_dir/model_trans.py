...
class _ModelTrans(nn.Module):

    def __init__(self, embedding_dim, tagset_size, batch_size):
        super(_ModelTrans,self).__init__()
...
class ModelTrans():
    def __init__(self, train_data, test_data, embedding_dim, tagset_size, batch_size,k_fold):
        ...
        self.model = _ModelTrans(self.embedding_dim,self.tagset_size,self.batch_size).cuda()
    ...
    def train(self):
        ...
        collate_fn = Collator(train_edu=False, train_trans=True, train_rlat=False)
        ...
        for epoch in range(10): 
            ...
            torch.save(self.model.state_dict(),'saved_model/model_trans.pkl.{}'.format(epoch))

    def test(self):
        self.model.load_state_dict(torch.load("saved_model/model_trans.pkl.8")) # load pretrained model
        self.model.eval()
